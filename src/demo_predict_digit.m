function result = demo_predict_digit(imagePath)
% demo_predict_digit.m
% Command-line helper for predicting one image with all trained models.
%
% Usage:
...
if ~isfile(imagePath)
    imagePath = fullfile(rootDir, imagePath);
end
if ~isfile(imagePath)
...
[img28, ~] = preprocess_uploaded_image(imagePath);
result = predict_with_all_models(img28, assets);

fprintf('\n=== Demo Prediction Result ===\n');
fprintf('Image: %s\n', imagePath);
fprintf('SVM  -> Digit %d | Confidence: %.2f%%\n', result.svm.prediction, result.svm.confidence * 100);
fprintf('RF   -> Digit %d | Confidence: %.2f%%\n', result.rf.prediction, result.rf.confidence * 100);
fprintf('k-NN -> Digit %d | Confidence: %.2f%%\n', result.knn.prediction, result.knn.confidence * 100);
fprintf('==============================\n\n');
end

function assets = load_demo_assets(rootDir)
svmData = load(fullfile(rootDir, 'models', 'svm_model.mat'), 'svm_model');
rfData = load(fullfile(rootDir, 'models', 'rf_model.mat'), 'rf_model');
knnData = load(fullfile(rootDir, 'models', 'knn_model.mat'), 'knn_model');
...
stat_features(6) = corr2(img, fliplr(img));
stat_features(7) = corr2(img, flipud(img));
stat_features(8) = std(sum(img, 1));
stat_features(9) = std(sum(img, 2));
...
if isnan(value)
    error('Could not convert class label to numeric digit.');
end
end
